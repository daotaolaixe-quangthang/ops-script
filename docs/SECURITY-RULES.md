...
> The required value for every SSL vhost is: `max-age=63072000; includeSubDomains; preload`
> (minimum 2 years, preload-list eligible).

- `server_tokens off` must always be set.
- **HSTS must include `preload`** — minimum `max-age=63072000` (2 years) required for HSTS preload list eligibility. Applied per-SSL-vhost, not globally (see note above).
