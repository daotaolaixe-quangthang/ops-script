...
Define rough tiers based on RAM only. CPU cores are detected separately for worker/process sizing and display; they do not select `OPS_TIER`.

- **Tier S (small)**: < 1500 MB RAM
- **Tier M (medium)**: >= 1500 MB and < 5000 MB RAM
- **Tier L (large)**: >= 5000 MB RAM
...
OPS should detect actual RAM and map to the tier above. CPU cores remain separate tuning inputs after the tier is chosen.
